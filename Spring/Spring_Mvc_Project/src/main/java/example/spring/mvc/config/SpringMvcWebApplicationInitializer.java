...
implements WebApplicationInitializer{
	...
	public void onStartup(ServletContext appCtx) throws ServletException {
		...
		AnnotationConfigWebApplicationContext webAppCtx = 
				new AnnotationConfigWebApplicationContext();
		webAppCtx.setConfigLocation("example");
		//The above statement is equivalent to @ComponentScan(basePackages = "example"))
		DispatcherServlet frontControllerServlet = 
				new DispatcherServlet(webAppCtx);
		
		ServletRegistration.Dynamic registration = 
				appCtx.addServlet("frontController", frontControllerServlet);
